...
abstract_cn: "学习解决复杂任务序列——同时利用迁移和避免灾难性遗忘——仍是实现人类水平智能的关键障碍。渐进网络方法朝这个方向迈进一步：它们不会遗忘，通过横向连接利用先前学到的特征。我们在多种强化学习任务（Atari和3D迷宫游戏）上广泛评估该架构，表明其优于基于预训练和微调的基线。使用新颖的敏感度测量，我们证明迁移同时发生在学习策略的低层感知和高层控制层。"
keywords:
  - "[[Catastrophic forgetting]]"
  - "[[Continual learning]]"
