...
abstract_cn: "存储窗口闭合、读出延迟和栅极漏电是铁电场效应晶体管（FEFET）中三个关键可靠性挑战，传统上分别分析。本文通过专门实验和器件仿真详细研究了带有10nm HZO铁电层的Si-FEFET中这三种机制。结果揭示了陷阱产生与极化切换之间的相互作用，以及MW闭合、读出延迟和漏电流之间的关联。陷阱产生由极化切换加速，初始阶段（高达5x10^4次循环）主要发生在界面层（IL），这些慢陷阱负责MW恢复随读出延迟的降低和MW闭合。漏电流增加由HZO陷阱的产生控制，在IL高度退化后由内部电场重分布触发。最小化IL中慢（去）陷阱缺陷的形成对提高FEFET整体可靠性至关重要。"
keywords:
  - "[[Ferroelectric FET]]"
