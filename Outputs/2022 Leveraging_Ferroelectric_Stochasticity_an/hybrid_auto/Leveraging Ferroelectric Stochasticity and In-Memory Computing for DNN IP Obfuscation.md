...
abstract_cn: "随着物联网的兴起，深度神经网络被广泛应用于计算机视觉、医疗、社交媒体和国防等不同领域。DNN的硬件级架构可采用基于存内计算的设计，加载已训练好的DNN模型权重。然而，此类基于硬件的DNN系统容易受到模型窃取攻击，攻击者可逆向工程提取DNN模型权重。本文提出了一种能效防御技术，将基于铁电场效应晶体管（FeFET）的可重构物理不可克隆函数（PUF）与存内FeFET XNOR相结合，以阻止模型窃取攻击。我们利用铁电畴中的固有随机性构建PUF，当检测到对抗攻击时破坏神经网络权重。我们通过对图神经网络（一种特定类型的DNN）进行实验来展示该防御方案的有效性，这是首次评估GNN安全性的方案。我们研究了在五个不同生物信息数据集上，破坏GNN不同层权重以及两种FeFET-PUF误差模型对图分类精度退化的影响。我们证明该方法在短暂重写脉冲后，通过破坏GNN的任何一层都能成功降低图分类的推理精度。"
