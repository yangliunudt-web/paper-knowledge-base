...
keywords:
  ...
  - "[[Lifelong Learning]]"
abstract_cn: "深度模型（如CNN和Vision Transformer）在封闭世界的许多视觉任务中取得了令人瞩目的成就。然而，在不断变化的世界中，新类别不断出现，要求学习系统能够持续获取新知识。类增量学习（CIL）使学习器能够增量式地融合新类别的知识，并构建一个覆盖所有已见类别的通用分类器。当直接用新类别实例训练模型时，会出现一个致命问题——模型倾向于灾难性地遗忘旧类别的特征，导致性能严重下降。本文全面综述了类增量学习的最新进展，从多个方面总结现有方法，并在基准图像分类任务中对17种方法进行了严格统一的评估。此外，本文指出当前比较协议忽略了模型存储中内存预算的影响，可能导致不公平比较和有偏结果，因此倡导通过对齐内存预算以及内存无关的性能度量来实现公平比较。"
