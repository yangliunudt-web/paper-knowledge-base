...
abstract_cn: "自发结构弛豫是玻璃材料因其亚稳态性质的固有特性。对相变材料而言，由此产生的电阻随时间变化严重阻碍了神经形态计算应用。本文报道了从头算计算指导设计的非晶相变材料，由稳健的分子状基元组成，消除了非晶合金中导致弛豫和电阻漂移的关键结构成分。展示了非晶CrTe3薄膜在-200°C至165°C任何工作温度下几乎无电阻漂移，并通过光电混合方法实现了多级编码。进一步揭示了熔融淬火非晶CrTe3在电子器件中同样无漂移。通过将CrTe3集成到具有自动路径跟踪功能的车辆中验证了其应用潜力。该工作通过精心设计无序相变材料，为实现相变神经形态计算的必要性能提供了替代途径。"
keywords:
  ...
  - "[[CrTe3]]"
  - "[[相变存储器]]"
cite: "[1] Wang X, Wang R, Sun S, et al. Amorphous phase-change memory alloy with no resistance drift[J]. Nature Materials, 2025."
