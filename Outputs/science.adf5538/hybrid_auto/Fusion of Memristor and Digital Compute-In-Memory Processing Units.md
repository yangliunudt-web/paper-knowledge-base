...
abstract_cn: "人工智能（AI）边缘设备倾向于采用高容量非易失性存内计算（CIM）以实现高能效和快速唤醒响应，同时保持足够的精度。以往的工作大多基于忆阻器CIM，其受限于有限的耐久性而导致精度损失且不支持训练；或基于数字静态随机存取存储器（SRAM）CIM，其面临面积大和易失性存储的问题。我们报道了一种AI边缘处理器，采用忆阻器-SRAM CIM融合方案，同时利用数字SRAM CIM的高精度和阻变随机存取存储器忆阻器CIM的高能效和高存储密度。该方案还支持自适应本地训练，以适应个性化特征和用户环境。该融合处理器实现了高CIM容量、短唤醒响应延迟（392微秒）、高峰值能效（77.64 TOPS/W）和鲁棒的精度（<0.5%精度损失）。这项工作表明忆阻器技术已超越实验室开发阶段，具备AI边缘处理器的可制造性。"
