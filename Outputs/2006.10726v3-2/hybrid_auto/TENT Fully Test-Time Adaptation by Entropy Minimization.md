...
abstract: "A model must adapt itself to generalize to new and different data during testing.\
  \ In this setting of fully test-time adaptation the model has only the test data\
  \ and its own parameters. We propose to adapt by test entropy minimization (tent):\
  \ we optimize the model for confidence as measured by the entropy of its predictions.\
  ...
  \ new state-of-the-art error on ImageNet-C. Tent handles source-free domain adaptation\
  \ on digit recognition from SVHN to MNIST/MNIST-M/USPS, on semantic segmentation\
  \ from GTA to Cityscapes, and on the VisDA-C benchmark. These results are achieved\
  \ in one epoch of test-time optimization without altering training."
