...
abstract: "Attention-in-Memory (AiM), a computing-in-memory (CiM) design, is introduced to implement the attentional layer of Memory Augmented Neural Networks (MANNs). AiM consists of a memory array based on ferroelectric FETs (FeFETs) along with CMOS peripheral circuits implementing configurable functionalities, i.e., it can be dynamically changed from a ternary content-addressable memory (TCAM) to a general-purpose (GP) CiM. When compared to state-of-the-art accelerators, AiM achieves comparable end-to-end speed-up and energy for MANNs, with better accuracy (95.14% v.s. 92.21%, and 95.14% v.s. 91.98%) at iso-memory size, for a 5-way 5-shot inference task with the Omniglot dataset."
abstract_cn: "提出Attention-in-Memory（AiM）存内计算设计，用于实现记忆增强神经网络（MANN）的注意力层。AiM由基于铁电场效应晶体管（FeFET）的存储器阵列和CMOS外围电路组成，实现可配置功能——可从三态内容可寻址存储器（TCAM）动态切换为通用存内计算。与最先进加速器相比，AiM在MANN的5-way 5-shot Omniglot推理任务中以同等内存大小实现更高准确率（95.14% vs 92.21%和95.14% vs 91.98%），同时保持相当的端到端加速比和能效。"
keywords:
  - "[[Ferroelectric]]"
  - "[[In-memory computing]]"
