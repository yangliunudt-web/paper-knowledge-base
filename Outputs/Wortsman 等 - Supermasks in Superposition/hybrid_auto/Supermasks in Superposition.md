...
abstract_cn: "提出Supermasks in Superposition（SupSup）模型，能够顺序学习数千个任务而无灾难性遗忘。方法使用随机初始化的固定基础网络，为每个任务找到一个子网络（supermask）。若测试时给定任务标识，可检索正确的子网络；若未知，SupSup可通过基于梯度的优化推断任务——找到已学习supermasks的线性叠加使输出熵最小化，单个梯度步骤即可在2500个任务中识别正确mask。还展示了两个扩展：无需任务标识的训练和将增长的超掩码集合存储为固定大小Hopfield网络中的吸引子。"
cite: "[1] Wortsman M, Ramanujan V, Liu R, et al. Supermasks in Superposition[C]. NeurIPS, 2020."
aiSum: "SupSup模型基于固定随机网络的supermask叠加实现数千任务的无遗忘顺序学习。单步梯度即可从2500个任务中推断任务标识，支持无任务ID训练和Hopfield网络恒定大小存储。"
  - "[[Catastrophic forgetting]]"
  - "[[Task inference]]"
  - "[[Neural network pruning]]"
cite: "[1] Wortsman et al. Supermasks in Superposition[J]. NeurIPS, 2020."
