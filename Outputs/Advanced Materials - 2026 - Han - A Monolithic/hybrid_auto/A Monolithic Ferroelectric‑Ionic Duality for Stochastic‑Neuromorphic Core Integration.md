...
abstract_cn: "在数据密集型AI应用中，不确定性下的学习变得日益关键，需统一确定性和概率性功能的计算系统。结合稳定存储与可调随机性的硬件对此至关重要，但由于精确存储保持与可控随机变异之间的内在冲突，在微型架构中实现这种集成极具挑战性。本文展示了一种铪基铁电-离子双重性，在单一器件中集成随机编码和突触记忆。通过设计铁电界面将传统上被视为降低铪基铁电可靠性缺陷的氧空位重新利用为功能性离子组分，动态调制器件行为。由此产生的铁电-离子双模切换引入电压可调随机性并增强突触行为。该铁电-离子双重性具有完全CMOS兼容性和VLSI可扩展性，得益于晶圆级ALD铪生长。这些结果确立了将存储、随机性和学习能力统一在单一铁电平台中的新器件范式。"
keywords:
